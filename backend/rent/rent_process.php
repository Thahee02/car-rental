...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $total_amount = htmlspecialchars(trim($_POST['total-amount']));
    $security_deposit = htmlspecialchars(trim($_POST['security-deposit']));
    $balance_amount = htmlspecialchars(trim($_POST['balance-amount']));
    ...
    if (empty($cName) || empty($cPhone) || empty($cEmail) || empty($cAddress) || empty($cNic) || empty($carName) ||
        ...
        empty($expDate) || empty($payMethod) || empty($total_amount) || empty($security_deposit) || empty($balance_amount) || empty($billAddress)) {
        setModalData("<svg xmlns='http://www.w3.org/2000/svg' width='40' height='40' viewBox='0 0 24 24'><g fill='none' stroke='currentColor' stroke-linecap='round' stroke-linejoin='round' stroke-width='2'><path stroke-dasharray='64' stroke-dashoffset='64' d='M12 3c4.97 0 9 4.03 9 9c0 4.97 -4.03 9 -9 9c-4.97 0 -9 -4.03 -9 -9c0 -4.97 4.03 -9 9 -9Z'><animate fill='freeze' attributeName='stroke-dashoffset' dur='0.6s' values='64;0'/></path><path stroke-dasharray='8' stroke-dashoffset='8' d='M12 12l4 4M12 12l-4 -4M12 12l-4 4M12 12l4 -4'><animate fill='freeze' attributeName='stroke-dashoffset' begin='0.6s' dur='0.2s' values='8;0'/></path></g></svg>", "Error!", "Please fill in all the information.");
        exit();
    }
    ...
    p_location, d_location, license_no, country, exp_date, pay_method, card, card_no, cvv, total_amount, security_deposit, balance_amount, bill_address) 
    VALUES ('$userEmail', '$bookedDate', '$cName', '$cPhone', '$cEmail', '$cAddress', '$cNic', '$carName', '$startDate', '$endDate', 
    '$pLocation', '$dLocation', '$licenseNo', '$country', '$expDate', '$payMethod', '$card', '$cardNo', '$cvv', '$total_amount', '$security_deposit', '$balance_amount', '$billAddress')";

    // Execute the query
    if (mysqli_query($conn, $sql)) {
        $updateCarStatus = "UPDATE cars SET status='booked' WHERE car_number='$carName'";
        mysqli_query($conn, $updateCarStatus);
